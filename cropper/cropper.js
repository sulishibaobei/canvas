...
var render = {
  ...
  initCropBox: function initCropBox() {
    ...
    var cropBox = {
   //   width: canvas.width,
       width:options.cropBox[0],
       height: options.cropBox[1]
    };
    if (aspectRatio) {
   ...
   // cropBox.width = Math.max(cropBox.minWidth, cropBox.width);
    //cropBox.height = Math.max(cropBox.minHeight, cropBox.height);
    cropBox.width = Math.max(cropBox.minWidth, cropBox.width);
    cropBox.height = Math.max(cropBox.minHeight, cropBox.height);
...
var methods = {
  ...
  zoom: function zoom(ratio, _event) {
    ...
    var maxz=canvas.width * ratio / canvas.naturalWidth;
    if(maxz>2){
      maxz=2;
    }

    this.zoomTo(maxz, _event);
  },
